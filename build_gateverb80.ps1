...
$source = Join-Path $root "units\revfx\gateverb80"
$dest = Join-Path $sdk "platform\drumlogue\taureon-gateverb80"
$template = Join-Path $sdk "platform\drumlogue\dummy-revfx"
...
if (-not (Test-Path -LiteralPath $source -PathType Container)) {
    throw "Could not find GateVerb80 source directory: $source"
}

if (-not (Test-Path -LiteralPath $dest -PathType Container)) {
    if (-not (Test-Path -LiteralPath $template -PathType Container)) {
        throw "Could not find drumlogue revfx template: $template"
    }
...
docker run --rm `
    ...
    -h logue-sdk `
    $Image `
    /app/cmd_entry build drumlogue/taureon-gateverb80
