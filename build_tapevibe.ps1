...
$source = Join-Path $root "units\masterfx\tapevibe"
$dest = Join-Path $sdk "platform\drumlogue\taureon-tapevibe"
...
if (-not (Test-Path -LiteralPath $source -PathType Container)) {
    throw "Could not find TapeVibe source directory: $source"
}
...
docker run --rm `
    ...
    -h logue-sdk `
    $Image `
    /app/cmd_entry build drumlogue/taureon-tapevibe
