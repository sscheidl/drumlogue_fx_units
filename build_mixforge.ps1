...
$source = Join-Path $root "units\masterfx\mixforge"
$dest = Join-Path $sdk "platform\drumlogue\taureon-mixforge"
...
if (-not (Test-Path -LiteralPath $source -PathType Container)) {
    throw "Could not find MixForge source directory: $source"
}
...
docker run --rm `
    ...
    -h logue-sdk `
    $Image `
    /app/cmd_entry build drumlogue/taureon-mixforge
