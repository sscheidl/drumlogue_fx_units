...
$source = Join-Path $root "units\masterfx\drumshuffler"
$dest = Join-Path $sdk "platform\drumlogue\taureon-drumshuffler"
$template = Join-Path $sdk "platform\drumlogue\dummy-masterfx"
...
if (-not (Test-Path -LiteralPath $source -PathType Container)) {
    throw "Could not find DrumShuffler source directory: $source"
}

if (-not (Test-Path -LiteralPath $dest -PathType Container)) {
    if (-not (Test-Path -LiteralPath $template -PathType Container)) {
        throw "Could not find drumlogue masterfx template: $template"
    }
...
Copy-Item -LiteralPath `
    ...
    (Join-Path $source "drumshuffler.h"), `
...
docker run --rm `
    ...
    -h logue-sdk `
    $Image `
    /app/cmd_entry build drumlogue/taureon-drumshuffler
